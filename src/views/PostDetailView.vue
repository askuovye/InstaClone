...
const isCreator = computed(() => {
  ...
  return (
    authUser.value.id === post.value.user_id ||
    authUser.value.id === post.value.user?.id
  );
});
...
<template>
  <div class="post-view-root min-h-screen" style="background: #0d0e14">
    ...
    <div class="px-4 md:px-10 pt-5 pb-3">
      <button
        @click="$router.back()"
        class="back-btn group flex items-center gap-2.5 text-xs font-black tracking-widest text-white/30 hover:text-white transition-all duration-200 w-fit"
      >
        <span
          class="w-8 h-8 rounded-xl flex items-center justify-center border transition-all duration-200 group-hover:border-primary/40 group-hover:bg-primary/5 group-hover:text-primary"
          style="
            background: rgba(255, 255, 255, 0.04);
            border-color: rgba(227, 225, 235, 0.1);
          "
        >
    ...
    <div v-if="loading" class="px-4 md:px-10 pb-10">
      <div
        class="flex flex-col md:flex-row gap-0 rounded-3xl overflow-hidden border border-white/[0.06]"
        style="background: rgba(255, 255, 255, 0.02)"
      >
        <div
          class="flex-1 min-h-[60vw] md:min-h-[70vh] animate-pulse rounded-l-none"
        ...
        <div class="w-full md:w-[380px] p-6 flex flex-col gap-4">
          <div class="flex gap-3 items-center">
            ...
            ></div>
            <div
              class="h-4 w-32 rounded-lg animate-pulse"
              style="background: rgba(255, 255, 255, 0.06)"
            ></div>
          </div>
    ...
    >
      ...
      <p class="text-sm font-black tracking-widest" style="color: #ff6b6b">
        {{ error.toUpperCase() }}
      </p>
      <button
        @click="loadPost"
        class="mt-4 px-5 py-2 rounded-xl text-xs font-black tracking-widest text-white/50 hover:text-white transition-all duration-200"
        style="
    ...
    <!-- ── Post ── -->
    <div v-else-if="post" class="px-4 md:px-10 pb-10">
      <div
        class="post-shell flex flex-col md:flex-row gap-0 rounded-3xl overflow-hidden border border-white/[0.07]"
        style="background: rgba(255, 255, 255, 0.015)"
      >
        <!-- ═══ LEFT: Image ═══ -->
        <div
          class="image-panel flex-1 relative flex items-center justify-center overflow-hidden"
        ...
        >
          ...
          <!-- Scanlines overlay -->
          <div
            class="scanlines absolute inset-0 pointer-events-none z-10 opacity-10"
          ></div>
          ...
          <div
            class="corner-br absolute bottom-3 right-3 z-20 flex flex-col items-end"
          >
          ...
          <div
            class="absolute bottom-0 left-0 right-0 h-24 z-10 pointer-events-none"
            style="
        ...
        <!-- ═══ RIGHT: Panel ═══ -->
        <div
          class="side-panel w-full md:w-[370px] lg:w-[420px] flex flex-col border-t md:border-t-0 md:border-l border-white/[0.07]"
          style="background: rgba(10, 11, 16, 0.95)"
          :class="panelIn ? 'panel-in' : 'panel-out'"
        >
          <!-- ── Author header ── -->
          <div
            class="flex items-center gap-3 px-5 py-4 border-b border-white/[0.07]"
          >
            <div
              class="relative cursor-pointer flex-shrink-0 group/av"
              @click="router.push(`/profile/${post.user?.username}`)"
            >
              <div
                class="w-10 h-10 rounded-xl overflow-hidden flex items-center justify-center border transition-all duration-200 group-hover/av:border-primary/50"
                style="
                  background: rgba(255, 255, 255, 0.05);
                  border-color: rgba(227, 225, 235, 0.12);
                "
              >
                <img
              ...
              <div
                ...
                style="background: #05cc47; border-color: #0a0b10"
              ></div>
            </div>

            <div class="flex-1 min-w-0">
              <p
                class="font-black text-sm tracking-wider text-white cursor-pointer hover:text-primary transition-colors duration-200 truncate"
                @click="router.push(`/profile/${post.user?.username}`)"
              >
                {{ post.user?.username }}
              </p>
            ...
            <div class="relative">
              <button
                ...
                class="w-8 h-8 rounded-lg flex items-center justify-center text-white/25 hover:text-white hover:bg-white/5 transition-all duration-200"
          ...
          <div v-if="post.title" class="px-5 pt-4 pb-0">
            <h2
              class="font-black text-lg tracking-tight text-white leading-tight"
            >
              {{ post.title }}
            </h2>
          </div>
          ...
          <div class="px-5 py-4 flex-shrink-0">
            ...
            <div
              ...
              class="flex flex-wrap gap-x-1.5 gap-y-1 mt-3"
            >
              <span
                ...
                @mouseleave="
                  (e) => (e.target.style.color = 'rgba(5,204,71,0.7)')
                "
              >
                #{{ tag }}
              </span>
          ...
          <!-- ── Stats + Actions ── -->
          <div
            class="px-5 py-3 border-t border-b border-white/[0.07] flex items-center gap-5"
          >
            <!-- Like -->
            <button
              @click="toggleLike"
            ...
            >
              <span
                class="like-icon-wrap w-8 h-8 rounded-lg flex items-center justify-center transition-all duration-200"
                :style="
                  post.liked_by_me
                ...
                :class="
                  post.liked_by_me
                    ? ''
                    : 'group-hover/like:border-primary/30 group-hover/like:bg-primary/5'
                "
              >
                <i
                  class="text-base transition-all duration-200 group-hover/like:scale-125"
                  :class="
                    post.liked_by_me
                      ? 'bi bi-heart-fill text-primary like-pop'
                      : 'bi bi-heart text-white/40 group-hover/like:text-primary'
                  "
                ></i>
              </span>
              <span
                class="text-sm font-black tabular-nums transition-colors duration-200"
            ...
            <div class="flex items-center gap-2 text-white/40">
              <span
                ...
                style="
                  background: rgba(255, 255, 255, 0.04);
                  border: 1px solid rgba(227, 225, 235, 0.1);
                "
              >
          ...
          <!-- ── Meta strip ── -->
          <div
            v-if="post.views_count || post.created_at"
            class="px-5 py-2.5 border-b border-white/[0.07] flex items-center gap-4"
          >
            <span
              v-if="post.views_count"
              class="flex items-center gap-1.5 text-xs font-bold text-white/25"
            >
          ...
          <div class="flex-1 min-h-0 flex flex-col">
            <div class="px-5 pt-4 pb-2 flex items-center gap-2 flex-shrink-0">
              ...
              <h3 class="text-xs font-black tracking-widest text-white/30">
                COMMENTS
              </h3>
            </div>

            <div
              class="flex-1 overflow-y-auto px-5 pb-5 min-h-0 comments-scroll"
            >
              <PostCommentList
                :postId="post.id"
...
.image-in {
  animation: image-reveal 0.6s cubic-bezier(0.25, 0.46, 0.45, 0.94) both;
}
